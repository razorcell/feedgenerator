...
router.get('/', async (req, res) => {
  try {
    ...
    tools.logUserIP(req)
    let csv_obj = await functions.getStamUpdates(`Swedish`)
    let full_file_path = await tools.saveToCSV(csv_obj, 'StamSwedish')
    res.download(full_file_path)
  } catch (err) {
    tools.catchError(err, 'Stam-Swedish', undefined, undefined, 1)
