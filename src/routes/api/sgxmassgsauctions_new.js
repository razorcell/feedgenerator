...
router.get('/', async (req, res) => {
  try {
    ...
    tools.logUserIP(req)
    let csv_obj = await functions.getSGSAuctions_new_Updates(1)
    let full_file_path = await tools.saveToCSV(csv_obj, 'SGXMasSGSAuction')
    res.download(full_file_path)
  } catch (err) {
    tools.catchError(err, 'SGXMasSGSAuction', undefined, undefined, 1)
    res.status(500).send(`Server Error`)
  }
})

router.get('/getSGSAuctionsAuctionsList2021', async (req, res) => {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let Notices_files = await functions.get_SGS_Bonds_Auctions_new_List2021(`sgxmas`, 2)
    // clg(Notices_files);

    res.json(Notices_files)
  } catch (err) {
    tools.catchError(err, 'getAuctionsList', true, 'general', 2)
    res.status(500).send(`Server Error`)
  }
})
