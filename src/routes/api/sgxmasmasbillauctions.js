...
router.get('/', async (req, res) => {
  try {
    ...
    tools.logUserIP(req)
    let csv_obj = await functions.getSGXMASMASBillsAuctionsUpdates(1)
    let full_file_path = await tools.saveToCSV(csv_obj, 'SGXMasMasbillAuction')
    res.download(full_file_path)
  } catch (err) {
    tools.catchError(err, 'SGXMasMasbillAuction', undefined, undefined, 1)
    res.status(500).send(`Server Error`)
  }
})

router.get('/getMASBillsAuctionsList', async (req, res) => {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let Notices_files = await functions.getMASBillsAuctionsList(`sgxmas`, 2)
    // clg(Notices_files);

    res.json(Notices_files)
  } catch (err) {
    tools.catchError(err, 'getMASBillsAuctionsList', true, 'general', 2)
    res.status(500).send(`Server Error`)
  }
})
