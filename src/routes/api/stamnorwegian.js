...
router.get('/', async (req, res) => {
  try {
    ...
    tools.logUserIP(req)
    let csv_obj = await functions.getStamUpdates(`Norwegian`)
    let full_file_path = await tools.saveToCSV(csv_obj, 'StamNorwegian')
    res.download(full_file_path)
  } catch (err) {
    tools.catchError(err, 'Stam-Norwegian', undefined, undefined, 1)
...
router.post('/getStamSecuritiesForCountry', async (req, res) => {
  try {
    ...
    tools.catchError(err, 'getSecuritiesForCountry', true, 'general', 2)
    res.status(500).send(`Server Error`)
  }
})
