...
const functions = require(`../../services/functions/Finvestigate.js`)
...
router.get('/', async (req, res) => {
  try {
    ...
    tools.logUserIP(req)
    let csv_obj = await functions.getInvestigateUpdates(req.query.datevalue)
    let full_file_path = await tools.saveToCSV(csv_obj, 'investigate')
    res.download(full_file_path)
  } catch (err) {
    tools.catchError(err, 'investigate', undefined, undefined, 1)
    res.status(500).send(`Server Error`)
  }
})

router.post('/getAnnouncementsForDate', async (req, res) => {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let data = await functions.getAnnouncementsForDate(req.body.datevalue, 2)
    res.json(data)
  } catch (err) {
    tools.catchError(err, 'getAnnouncementsForDate', true, 'general', 2)
    res.status(500).send(`Server Error`)
  }
})
