...
const functions = require(`../../services/functions/Fsgxmasfrnauctions.js`)
...
//Simplify logging
const lg = tools.lg
let source = 'sgxmasfrnauctions'
let log_tab = 1

router.get('/', async function getSGXMASFRNAuctionsUpdates(req, res) {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let csv_obj = await functions.getSGXMASFRNAuctionsUpdates(source, log_tab)
...
router.get('/getFRNAuctionsList', async function getFRNAuctionsList(req, res) {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let Notices_files = await functions.getFRNAuctionsList(source, log_tab)
    // clg(Notices_files);
