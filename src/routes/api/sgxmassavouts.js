...
router.get('/', async (req, res) => {
  try {
    ...
    tools.logUserIP(req)
    let csv_obj = await functions.getSGXMASSavingsBondsUpdates(1)
    let full_file_path = await tools.saveToCSV(csv_obj, 'SGXMASSavingsBonds')
    res.download(full_file_path)
  } catch (err) {
    tools.catchError(err, 'SGXMASSavingsBonds', undefined, undefined, 1)
    res.status(500).send(`Server Error`)
  }
})

router.post('/getSavingsBondsList', async (req, res) => {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    req.body['source'] = `sgxmas`
    let data = await functions.getSavingsBondsList(req.body, 2)
    res.json(data)
  } catch (err) {
    tools.catchError(err, 'getSavingsBondsList', true, 'general', 2)
    res.status(500).send(`Server Error`)
  }
})
