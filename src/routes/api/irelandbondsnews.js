...
const functions = require(`../../services/functions/Firelandbondsnews.js`)
...
router.post('/getNoticesFromPage', async (req, res) => {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let data = await functions.getNoticesFromPage(req.body.page_id, 2)
    res.json(data)
  } catch (err) {
...
router.post('/getArticleBody', async (req, res) => {
  try {
    ...
    let Notice = {
      ...
      issuer: '',
      title: 'Title',
      url: 'https://www.ntma.ie/news/ireland-sells-1-5-billion-of-bonds-maturing-in-2027-2030-and-2035-by-auction',
    }
