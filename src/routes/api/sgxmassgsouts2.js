...
router.get('/', async function getSGSBondsListSecondTable(req, res) {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let csv_obj = await functions.getSGSBondsListSecondTable(source, log_tab)
    let full_file_path = await tools.saveToCSV(csv_obj, 'SGXMasSGSOutstandingsecondTable')
...
router.get('/getSGSBondsListSecondTable', async function getSGSBondsListSecondTable(req, res) {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let data = await functions.getSGSBondsListSecondTable(source, log_tab)
    res.json(data)
  } catch (err) {
    tools.catchError(err, tools.gFName(new Error()), undefined, undefined, 1, source)
    res.status(500).send(`Server Error`)
