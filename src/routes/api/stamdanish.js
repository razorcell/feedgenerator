...
router.get('/', async (req, res) => {
  try {
    ...
    tools.logUserIP(req)
    let csv_obj = await functions.getStamUpdates(`Danish`)
    let full_file_path = await tools.saveToCSV(csv_obj, 'StamDanish')
    res.download(full_file_path)
  } catch (err) {
    tools.catchError(err, 'Stam-Danish', undefined, undefined, 1)
