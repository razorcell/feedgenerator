...
const functions = require(`../../services/functions/Fsaudidividends.js`)
...
//Simplify logging
const lg = tools.lg
let source = 'saudidividends'
let log_tab = 1

router.get('/', async function getSaudiDividendsUpdates(req, res) {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let csv_obj = await functions.getSaudiDividendsUpdates(source, log_tab)
...
router.get('/getSecuritiesList', async function getSecuritiesList(req, res) {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let Notices_files = await functions.getSecuritiesList(source, log_tab)
    // clg(Notices_files);
...
router.get('/getOneSecurityDividends', async function getSecuritiesList(req, res) {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let Trs = await functions.getOneSecurityDividends(source, req.body, log_tab)
    // clg(Notices_files);
    res.json(Trs)
  } catch (err) {
    tools.catchError(err, tools.gFName(new Error()), undefined, undefined, 1, source)
