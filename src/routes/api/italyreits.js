...
const functions = require(`../../services/functions/Fitalyreits.js`)
const tools = require(`../../services/tools`)

const router = express.Router()
module.exports = router

let source = 'italyreits'
let log_tab = 1
// @route GET /
// @desc Get Course direct free float
// @access Public
router.get('/', async function getItalianReitsUpdates(req, res) {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let csv_obj = await functions.getItalianReitsUpdates(source, log_tab)
    // clg(csv_obj);
