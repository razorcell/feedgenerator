...
import {
  getMexicoREITSUpdates,
  getAllCompanies,
  getAllSecurities,
  getOneSecurityDetails,
  getAllSecuritiesForOneCompany,
  getListingDateForOneCompany,
} from '../../services/functions/Fmexicoreits'
import { logUserIP, saveToCSV, catchError } from '../../services/tools'

const router = express.Router()
export default router

const source = 'mexicoreits'
const logTab = 1
const sourceConfig: MexicoREITSSourceConfig = global.gfinalConfig[source]

router.get('/', async (req, res) => {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    logUserIP(req)
    let csv_obj = await getMexicoREITSUpdates(source, sourceConfig, logTab)
