...
const functions = require(`../../services/functions/Flesechos.js`)
...
router.get('/', async function getLesEchosUpdates(req, res) {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    logUserIP(req)
    let csv_obj = await functions.getLesEchosUpdates(source, log_tab)
