...
router.get('/', async (req, res) => {
  try {
    ...
    tools.logUserIP(req)
    let csv_obj = await functions.getSGXMASTbillsAuctionsUpdates(1)
    let full_file_path = await tools.saveToCSV(csv_obj, 'SGXMasTbillAuction')
    res.download(full_file_path)
  } catch (err) {
    tools.catchError(err, 'SGXMasTbillAuction', undefined, undefined, 1)
    res.status(500).send(`Server Error`)
  }
})

router.get('/getTBillsAuctionsList', async (req, res) => {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let Notices_files = await functions.getTBillsAuctionsList(`sgxmas`, 2)
    // clg(Notices_files);

    res.json(Notices_files)
  } catch (err) {
    tools.catchError(err, 'getTBillsAuctionsList', true, 'general', 2)
    res.status(500).send(`Server Error`)
  }
})
