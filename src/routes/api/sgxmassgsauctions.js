...
router.get('/', async (req, res) => {
  try {
    ...
    tools.logUserIP(req)
    let csv_obj = await functions.getSGXMASSGSAuctionsUpdates(1)
...
router.get('/getSGSAuctionsList', async (req, res) => {
  try {
    req.setTimeout(20 * 60 * 60 * 1000) //force timeout to 4 hours
    tools.logUserIP(req)
    let Notices_files = await functions.getSGSAuctionsList(`sgxmas`, 2)
    // clg(Notices_files);
