...
export async function getMexicoETFUpdates(
  source: string,
  sourceConfig: MexicoETFSourceConfig,
  logTab: number
) {
  try {
    lg(`START - getMexicoETFUpdates`, logTab, 'info', source)
    let allData = []
    ...
    lg(`Extracted ${allData.length} ETFs from Mexico`, logTab + 1, 'info', source)
    return allData
  } catch (err) {
    catchError(err, 'getMexicoETFUpdates', undefined, undefined, logTab, source)
    throw new Error(err)
  }
}

export async function getAllCompanies(source: string, logTab: number): Promise<MexicoETFCompany[]> {
  try {
    let page = ``
    page = await Ffetch.down(
      {
        source,
        method: 'GET',
        url: `https://www.bmv.com.mx/en/Grupo_BMV/Informacion_de_emisora/_rid/541/_mto/3/_mod/doSearch?idTipoMercado=CGEN_CAPIT&idTipoInstrumento=CGEN_ELTRA&idTipoEmpresa=&idSector=&idSubsector=&idRamo=&idSubramo=&random=${getRandomInt(
          1000,
          9999
        )}`,
        id: `getAllETFs`,
    ...
    let allCompanies: MexicoETFCompany[] = obj.response.resultado.map(
      (sec: {
        idEmisora: any
        idTpEmpresa: any
        claveEmisora: any
...
export async function addDefaults(source: string, all_data: MexicoETFCompany[], logTab: number) {
  try {
    all_data = all_data.map((row: MexicoETFCompany) => {
      row.FYE = '31/12'
...
export async function addURLsTags(source: string, all_data: MexicoETFCompany[], logTab: number) {
  try {
    all_data = all_data.map((row: MexicoETFCompany) => {
...
export async function getAllSecuritiesDetails(
  source: string,
  allSecurities: MexicoETFCompany[],
  sourceConfig: MexicoETFSourceConfig,
  logTab: number
): Promise<MexicoETFCompany[]> {
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      all_data = all_data.concat(
        await Promise.all(
          chunks[i].map(async (security: MexicoETFCompany) => {
...
export async function getAllSecurities(
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      all_data = all_data.concat(
        await Promise.all(
          chunks[i].map(async (company: MexicoETFCompany) => {
