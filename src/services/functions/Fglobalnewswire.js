...
async function getNoticesForPage(source, pageID, log_tab) {
  try {
    let res = await Faxios.down(
      {
        ...
        source,
        url: `https://www.globenewswire.com/search/exchange/paris%252520stock%252520exchange?page=${pageID}`,
      },
      log_tab + 1
    )
    ...
    let $ = articles.$
    articles = articles.contents.map((a_tag, indx) => {
      return {
        url: $(a_tag).attr('href'),
        title: $(a_tag).text(),
      }
...
async function getOneNoticeDetails(source, notice, log_tab) {
  try {
    let page = ``
    notice.url = `https://www.globenewswire.com${notice.url}`
    page = (
      await Faxios.down(
        {
          source,
    ...
    let timeTag = await scraping.getOneTag('span.article-published', page, 'text', log_tab + 1, source)
    if (!timeTag.found) {
      lg(`Could not find Time tag`, log_tab + 1, 'warn', source)
    } else {
      notice.date = moment(timeTag.content.replace(' ET', ''), 'MMM DD, YYYY HH:mm').format(
        'YYYYMMDD HH:mm:ss'
      )
    }
    let body = await scraping.getOneTag('.main-scroll-container', page, 'text', log_tab + 1, source)
    if (!body.found) {
