...
export async function getMexicoREITSUpdates(
  source: string,
  sourceConfig: MexicoREITSSourceConfig,
  logTab: number
) {
  try {
    lg(`START - getMexicoREITSUpdates`, logTab, 'info', source)
    let allData: MexicoREITSCompany[] = []
    let allCompanies = await getAllCompanies(source, logTab + 1)
    ...
    lg(`Extracted ${allData.length} REITs from Mexico`, logTab + 1, 'info', source)
    return allData
  } catch (err) {
    catchError(err, 'getMexicoREITSUpdates', undefined, undefined, logTab, source)
    throw new Error(err)
  }
}

export async function getAllCompanies(source: string, logTab: number): Promise<MexicoREITSCompany[]> {
  try {
    let page = ``
    page = await Ffetch.down(
      {
        source,
        method: 'GET',
        url: `https://www.bmv.com.mx/en/Grupo_BMV/Informacion_de_emisora/_rid/541/_mto/3/_mod/doSearch?idTipoMercado=CGEN_CAPIT&idTipoInstrumento=CGEN_ELFI&idTipoEmpresa=&idSector=&idSubsector=&idRamo=&idSubramo=&random=${getRandomInt(
          1000,
          9999
        )}`,
        id: `getAllETFs`,
    ...
    let allCompanies: MexicoREITSCompany[] = obj.response.resultado.map(
      (sec: {
        idEmisora: any
        idTpEmpresa: any
        claveEmisora: any
        ...
        return {
          ...
          idTpEmpresa: sec.idTpEmpresa,
          claveEmisora: sec.claveEmisora,
          razonSocial: sec.razonSocial,
          razonComercial: sec.razonComercial,
...
export async function addDefaults(source: string, all_data: MexicoREITSCompany[], logTab: number) {
  try {
    all_data = all_data.map((row: MexicoREITSCompany) => {
      row.FYE = '31/12'
      row.ticker = `${row.claveEmisora}${row.idEmision}`
...
export async function addURLsTags(source: string, all_data: MexicoREITSCompany[], logTab: number) {
  try {
    all_data = all_data.map((row: MexicoREITSCompany) => {
...
export async function getAllSecuritiesDetails(
  source: string,
  allSecurities: MexicoREITSCompany[],
  sourceConfig: MexicoREITSSourceConfig,
  logTab: number
): Promise<MexicoREITSCompany[]> {
  try {
    ...
    let all_data: MexicoREITSCompany[] = []
    for (let i = 0; i < chunks.length; i++) {
      ...
      all_data = all_data.concat(
        await Promise.all(
          chunks[i].map(async (security: MexicoREITSCompany) => {
...
export async function getAllSecurities(
  source: string,
  allCompanies: MexicoREITSCompany[],
  sourceConfig,
  logTab: number
): Promise<MexicoREITSCompany[]> {
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      all_data = all_data.concat(
        await Promise.all(
          chunks[i].map(async (company: MexicoREITSCompany) => {
